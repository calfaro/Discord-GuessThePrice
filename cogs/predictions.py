...
class Predictions(commands.Cog):
    ...
    @commands.slash_command(
        name="openbets",description="(Admin) Starts an event, enables /placebets command",
    )
    ...
    async def open_bets_command(self, inter, currency:str='USD', url: str=None):
        ...
        if data['open'] == True:
            await inter.response.send_message(f'There is already a prediction event in progress', ephemeral=True)
            return
        ...
        embed = Embed(title='Bets are open for the following event:', description=f'**Required Currency**: {currency}\n\n**{title}**\n{desc} -- ([Read more...]({url}))')
    ...
    async def closebets_command(self, inter):
        ...
        if data['open']:
            ...
            await inter.response.send_message(f'Bets are already closed.',ephemeral=True)
    ...
    async def prediction(self, inter, currency: str, price: float):
        ...
        if currency != req_currency and req_currency is not None:
            await inter.response.send_message(
                f"Your prediction must be in the correct currency: **{req_currency}**. Please try again.", ephemeral=True
            )
            return
    ...
    async def final_price_command(self, inter, currency: str, real_price: float):
        ...
        req_currency = data["currency"]
        if currency != req_currency and req_currency is not None:
            await inter.response.send_message(f'Please submite the final price with the correct currency: {req_currency}', ephemeral=True)
            return
